...
function List() {
  ...
  useEffect(() => {
    fetchChatList();
    const interval = setInterval(fetchChatList, 3000); // 3초마다 새로고침
              ...
              >
                <div className="profile">
                  <div className="avatar">
                    <img src={profileImg} alt="프로필" style={{ width: '100%', height: '100%', borderRadius: '50%' }} />
                  </div>
                </div>
                <div className="chat-info">
                  <div className="chat-username">
                    {chat.username || 'Unknown'} <span>{chat.userId}</span>
                  </div>
                  <div className="chat-message">{chat.lastMessage || '대화 없음'}</div>
                </div>
